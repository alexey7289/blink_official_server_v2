...
document.addEventListener('DOMContentLoaded', () => {
	...
	// Функция рендера дополнительных полей размеров
	function renderExtraDims(draw) {
		...
		// Вешаем валидацию на каждое новое поле
		dims.forEach(f => {
	...
	function validateDimensionsAndCheckButton() {
		...
		function checkField(inputElement, value) {
			...
			if (isValid) {
				...
				// Подсвечиваем шаг резки как причину ошибки
				if (stepInput) {
					stepInput.error = true;
					stepInput.errorText = "Проверь шаг";
				}
			}
		}
		...
		// Проверяем extra-dims поля если они есть
	...
	function sendDraw(id) {
	    ...
	    if (draw) renderExtraDims(draw); // ← добавь
